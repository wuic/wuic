...
public class PathMap<O> extends HashMap<String,O>
{
    ...
    /* --------------------------------------------------------------- */
    ArrayTernaryTrie<MappedEntry<O>> _prefixMap=new ArrayTernaryTrie<MappedEntry<O>>(false);
    ArrayTernaryTrie<MappedEntry<O>> _suffixMap=new ArrayTernaryTrie<MappedEntry<O>>(false);
            ...
            if (entry.getKey().equals(spec))
            {
                ...
                else if (spec.endsWith("/*"))
                {
                    ...
                    while (!_prefixMap.put(mapped,entry))
                        _prefixMap=new ArrayTernaryTrie<MappedEntry<O>>((ArrayTernaryTrie<MappedEntry<O>>)_prefixMap,1.5);
                }
                else if (spec.startsWith("*."))
                {
                    String suffix=spec.substring(2);
                    while(!_suffixMap.put(suffix,entry))
                        _suffixMap=new ArrayTernaryTrie<MappedEntry<O>>((ArrayTernaryTrie<MappedEntry<O>>)_suffixMap,1.5);
                }
                else if (spec.equals(SLASH))
                {
                    if (_nodefault)
    ...
    public MappedEntry<O> getMatch(String path)
    {
        ...
        // prefix search
        int i=l;
        final ArrayTernaryTrie<MappedEntry<O>> prefix_map=_prefixMap;
        while(i>=0)
        {
        ...
        // Extension search
        i=0;
        final ArrayTernaryTrie<MappedEntry<O>> suffix_map=_suffixMap;
    ...
    {
        MappedEntry<O> entry;
        List<MappedEntry<O>> entries=new ArrayList<MappedEntry<O>>();
        ...
        // Extension search
        i=0;
        final ArrayTernaryTrie<PathMap.MappedEntry<O>> suffix_map=_suffixMap;
        while ((i=path.indexOf('.',i+1))>0)
        {
            entry=suffix_map.get(path,i+1,l-i-1);
            if (entry!=null)
    ...
    {
        _exactMap.clear();
        _prefixMap=new ArrayTernaryTrie<MappedEntry<O>>(false);
        _suffixMap=new ArrayTernaryTrie<MappedEntry<O>>(false);
        _default=null;
        _defaultSingletonList=null;
        _prefixDefault=null;
        super.clear();
